...
export default function FormFields({ field, tag, title, type = "", options = [], required, value, onChangeValue }) {
  ...
  if (field === "input") {
    return (
      <div className="relative">
        <input
          ...
          className={
            type === "file"
              ? "block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-cyan-950 dark:border-gray-600 dark:placeholder-gray-400"
              : "peer placeholder-transparent h-10 w-full border-b-2 border-gray-300 text-gray-900 dark:bg-cyan-950 dark:text-gray-300 focus:outline-none focus:borer-rose-600"
          }
          placeholder={title}
          onBlur={handleOnBlur}
    ...
    return (
      <div className="relative">
        <label htmlFor={tag} className="block mb-2 text-sm font-medium text-gray-600 dark:text-gray-300">
          Select {title}
        </label>
        <select
          id={tag}
